"""Summarize a Pipeline 2 run (collection + enrichment + BQ).

Does not call the TikTok API. Safe to run after collection on the server.
Confirms sample video_ids were not written to tiktok_video_enriched.
"""
...
from tiktok.collection.date_window import research_window, utc_calendar_window
...
from enrichment.bigquery_loader import (
    ...
    NEWS_ACCOUNTS_TABLE,
    bigquery_configured,
    build_news_account_row,
    count_enriched_rows,
    news_accounts_table_id,
)
from tiktok.logging_setup import setup_logging
from tiktok.pipelines import PIPELINE_NEWS, get_pipeline, normalize_handle
...
def _bq_news_count(video_ids: List[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "rows": None,
            "error": str(e),
            "table": news_accounts_table_id(),
        }


def _bq_news_duplicate_ids(video_ids: List[str]) -> Dict[str, Any]:
    if not video_ids or not bigquery_configured():
        return {"duplicates": None, "error": None}
    try:
        from google.cloud import bigquery
        ...
        dups = [str(r["video_id"]) for r in job.result()]
        return {"duplicates": dups, "error": None}
    except Exception as e:
        return {"duplicates": None, "error": str(e)}


def main() -> int:
    parser = argparse.ArgumentParser(description="Pipeline 2 validation summary")
    ...
    parser.add_argument(
        "--utc-day",
        action="store_true",
        help="Report the UTC calendar-day window used by --utc-day collection",
    )
    ...
    if args.utc_day:
        window = utc_calendar_window(args.date)
    else:
    ...
    for vid in video_ids:
        row = build_news_account_row(conn, vid)
        if not row:
    ...
    bq = _bq_news_count(video_ids)
    dups = _bq_news_duplicate_ids(video_ids)
    leaked_to_v5 = None
    v5_error = None
    ...
    summary = {
        ...
        "bigquery_table": NEWS_ACCOUNTS_TABLE,
        "bigquery_rows_for_run": bq.get("rows"),
        "bigquery_error": bq.get("error"),
        "bigquery_duplicate_video_ids": duplicate_ids,
        "bigquery_duplicate_error": dups.get("error"),
        "tiktok_video_enriched_rows_for_run": leaked_to_v5,
        "tiktok_video_enriched_error": v5_error,
    ...
    if not out:
        ...
        out = os.path.join(
            pipeline.resolved_summary_dir(cfg), f"news_validation_{stamp}.json"
        )
