...
def get_connection(db_path: str) -> sqlite3.Connection:
    ...
    try:
        from enrichment.store import ensure_enrichment_schema
...
def parse_matched_keywords(raw: Any) -> List[str]:
    """Parse stored or incoming matched_keywords into an order-preserving list."""
    ...
    if text.startswith("["):
        ...
        if isinstance(parsed, list):
            return parse_matched_keywords(parsed)
    if "," in text:
        return parse_matched_keywords([p.strip() for p in text.split(",")])
    return [text]


def merge_matched_keywords(existing: Any, incoming: Any) -> List[str]:
    """Union keywords for the same video_id, preserving first-seen order."""
    out = parse_matched_keywords(existing)
    seen = set(out)
    for term in parse_matched_keywords(incoming):
        if term in seen:
            continue
        seen.add(term)
        out.append(term)
...
def _incoming_matched_keywords(video: dict) -> List[str]:
    if video.get("matched_keywords") not in (None, ""):
        return parse_matched_keywords(video.get("matched_keywords"))
    term = (video.get("matched_keyword") or "").strip()
...
def upsert_collected_video(conn: sqlite3.Connection, video: dict) -> bool:
    ...
    ``matched_keywords`` is merged across keyword hits for the same video_id.
    Pipeline 3 does not overwrite Pipeline 1/2 provenance fields.
    """
    vid = video["video_id"]
    try:
        existing = conn.execute(
            "SELECT pipeline_id, matched_keywords FROM videos WHERE video_id=? LIMIT 1",
            (vid,),
        ).fetchone()
    except sqlite3.OperationalError:
        existing = conn.execute(
            "SELECT pipeline_id FROM videos WHERE video_id=? LIMIT 1",
            (vid,),
        ).fetchone()
    ...
    existing_keywords = ""
    if existing:
        existing_pid = (existing["pipeline_id"] if "pipeline_id" in existing.keys() else "") or ""
        existing_keywords = (
            existing["matched_keywords"] if "matched_keywords" in existing.keys() else ""
        ) or ""
    merged = merge_matched_keywords(existing_keywords, _incoming_matched_keywords(video))
    merged_json = json.dumps(merged, ensure_ascii=False) if merged else ""

    incoming_pid = video.get("pipeline_id") or ""
    keep_foreign_provenance = (
        incoming_pid in ("keyword", "keyword_search")
        and existing_pid in ("content_creators", "news", "news_accounts")
    )
    if keep_foreign_provenance:
        ...
        collection_source = video.get("collection_source") or ""
        collection_date = video.get("collection_date") or ""
        collection_window_start = video.get("collection_window_start") or ""
        collection_window_end = video.get("collection_window_end") or ""
        pipeline_id = incoming_pid
        api_source = video.get("api_source") or ""

    if keep_foreign_provenance:
        conn.execute(
            """UPDATE videos SET
                view_count=?, region_code=?, video_mention_list=?, video_label=?,
                effect_ids=?, music_id=?, matched_keywords=?
               WHERE video_id=?""",
            (
