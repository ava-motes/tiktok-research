...
def get_access_token() -> str:
    ...
    expires_in = data.get("expires_in") or nested.get("expires_in") or 7200
    if not token:
        err = data.get("error")
        if isinstance(err, dict):
            code = err.get("code") or err.get("error") or "unknown_error"
            msg = err.get("message") or err.get("error_description") or ""
        else:
            code = err or f"http_{resp.status_code}"
            msg = data.get("error_description") or data.get("message") or ""
        raise RuntimeError(
            f"TikTok OAuth did not return access_token "
            f"(code={code}; {msg})".strip()
        )
