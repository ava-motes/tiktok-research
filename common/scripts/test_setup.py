...
def main() -> int:
    ...
    parser.add_argument("--config", default="common/config.yaml", help="Path to config YAML")
    ...
    try:
        handles = cfg.get_handles("batch_test")[:2]
    ...
    if not handles:
        logger.error(
            "No handles in 'batch_test' group — add handles under "
            "handle_groups.batch_test in common/config.yaml for user/info validation."
        )
