...
## Isolated Pipeline 1 table

`content_creators` is a separate research table. Daily command:
`scripts/run_content_creators.py --date YYYY-MM-DD --sample`.
Operator glossary + SSH runbook: [`COLLECTION_PIPELINES.md`](COLLECTION_PIPELINES.md)
(*How to run P1 / P2 / P3 from your laptop*). The v5.0
`tiktok_video_enriched` path is unchanged.

## Isolated Pipeline 2 table

`news` is a separate research table. Daily command:
`scripts/run_news.py --date YYYY-MM-DD --sample`. Handle list is 137 unique
accounts from Institutional Handles 08252026.xlsx. Uses dedicated `NEWS_API_*`
credentials.

## Isolated Pipeline 3 table

`keyword` is a separate research table. Daily command:
`scripts/run_keyword.py --date YYYY-MM-DD --sample`. Canonical list is
263 terms from `march_news_keywords.csv`. Sample is `news`, `trump`, `tsa`,
`ice`, `netanyahu`. Uses dedicated `KEYWORD_SEARCH_API_*` credentials and the
existing Whisper / OCR / emoji workers. Do not run the full 263-term collection
until the sample is reviewed.
